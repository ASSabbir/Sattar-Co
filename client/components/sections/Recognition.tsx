...
export default function Recognition() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.utils.toArray<HTMLElement>(".directory-name").forEach((el, i) => {
        gsap.fromTo(
          el,
          { opacity: 0.15, x: -12 },
          {
  ...
  return (
    <section ref={rootRef} className="grain bg-navy py-24 md:py-36">
      <div className="max-w-content mx-auto px-6 md:px-10">
        <SectionLabel label="Recognition" index="02" light className="mb-8" />
          ...
          <div className="lg:col-span-8 flex flex-col gap-10">
            {recognition.quotes.map((q, i) => (
              ...
              >
                ...
                <p className="eyebrow text-gold mt-4">{q.source}</p>
              </div>
            ))}
          </div>
        </div>
